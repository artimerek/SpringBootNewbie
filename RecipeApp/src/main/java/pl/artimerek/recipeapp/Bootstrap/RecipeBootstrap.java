package pl.artimerek.recipeapp.Bootstrap;


import org.springframework.context.ApplicationListener;
import org.springframework.context.event.ContextRefreshedEvent;
import org.springframework.stereotype.Component;
import pl.artimerek.recipeapp.Domain.*;
...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private final UnitRepo unitOfMeasureRepository;

    public RecipeBootstrap(CategoryRepo categoryRepository, RecipeRepo recipeRepository, UnitRepo unitOfMeasureRepository) {
    ...
    private List<Recipe> getRecipes() {
        ...
        //get UOMs
        Optional<Unit> eachUomOptional = unitOfMeasureRepository.findByDescription("Each");

        if(!eachUomOptional.isPresent()){
            throw new RuntimeException("Expected Unit Not Found");
        }

        Optional<Unit> tableSpoonUomOptional = unitOfMeasureRepository.findByDescription("Tablespoon");

        if(!tableSpoonUomOptional.isPresent()){
            throw new RuntimeException("Expected Unit Not Found");
        }

        Optional<Unit> teaSpoonUomOptional = unitOfMeasureRepository.findByDescription("Teaspoon");

        if(!teaSpoonUomOptional.isPresent()){
            throw new RuntimeException("Expected Unit Not Found");
        }

        Optional<Unit> dashUomOptional = unitOfMeasureRepository.findByDescription("Dash");

        if(!dashUomOptional.isPresent()){
            throw new RuntimeException("Expected Unit Not Found");
        }

        Optional<Unit> pintUomOptional = unitOfMeasureRepository.findByDescription("Pint");

        if(!pintUomOptional.isPresent()){
            throw new RuntimeException("Expected Unit Not Found");
        }

        Optional<Unit> cupsUomOptional = unitOfMeasureRepository.findByDescription("Cup");
        ...
        Optional<Unit> gramsOptional = unitOfMeasureRepository.findByDescription("Grams");

        if (!gramsOptional.isPresent()) {
            throw new RuntimeException("Expected Unit Not Found");
        }

        Optional<Unit> militersOptional = unitOfMeasureRepository.findByDescription("Militers");
        ...
        //get optionals
        Unit eachUom = eachUomOptional.get();
        Unit tableSpoonUom = tableSpoonUomOptional.get();
        Unit teapoonUom = tableSpoonUomOptional.get();
        Unit dashUom = dashUomOptional.get();
        Unit pintUom = pintUomOptional.get();
        Unit cupsUom = cupsUomOptional.get();
        Unit gramsUom = gramsOptional.get();
        Unit miliUom = militersOptional.get();
        ...
        Optional<Category> homemadeCategoryOptional = categoryRepository.findByDescription("Homemade");
        ...
        Category homemadeCategory = homemadeCategoryOptional.get();

        // Pancakes

        Recipe pancake = new Recipe();
        pancake.setDescription("Pancakes");
        pancake.setPrepTime(20);
        pancake.setCookTime(10);
        pancake.setDifficulty(Difficulty.EASY);
        pancake.setDirections("n a large bowl, mix flour, sugar, baking powder and salt. Make a well in the center, and pour in milk, egg and oil. Mix until smooth.\n" +
                "\n" +
                "Step 2\n" +
                "Heat a lightly oiled griddle or frying pan over medium high heat. Pour or scoop the batter onto the griddle, using approximately 1/4 cup for each pancake. Brown on both sides and serve hot.");
        Notes pancakeNotes = new Notes();
        pancakeNotes.setRecipeNotes("Enjoy you cant make it in wrong way");
        pancake.setNotes(pancakeNotes);

        pancake.getIngredients().add(new Ingredient("flour",new BigDecimal(125),gramsUom));
        pancake.getIngredients().add(new Ingredient("salt",new BigDecimal(1),teapoonUom));
        pancake.getIngredients().add(new Ingredient("egg",new BigDecimal(1),eachUom));
        pancake.getIngredients().add(new Ingredient("milk",new BigDecimal(330),miliUom));
        pancake.getIngredients().add(new Ingredient("butter",new BigDecimal(1),tableSpoonUom));
        pancake.getIngredients().add(new Ingredient("sugar",new BigDecimal(1),teapoonUom));

        pancake.getCategories().add(homemadeCategory);
        recipes.add(pancake);
